...
import com.revolo.lock.util.MiscUtil;
...
public class WifiCircleProgress extends View {

    private static final String TAG = WifiCircleProgress.class.getSimpleName();
    ...
    private int[] mGradientColors = {0xFF5FE2E7, 0xaa177BD1, 0xFF5FE2E7};
    private int[] mBgGradientColors = {0xaaF5F5F5, 0xFFDBDBDB, 0xaaF5F5F5};
    //当前进度，[0.0f,1.0f]
    ...
    private float maxArcWidth;

    public WifiCircleProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
...
//        mBgArcPaint.setColor(mBgArcColor);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //求圆弧和背景圆弧的最大宽度
        maxArcWidth = Math.max(mArcWidth, mBgArcWidth);
        //求最小值作为实际值
    ...
    private void drawArc(Canvas canvas) {
        ...
        double x = mCenterPoint.x + Math.cos(radian) * (mRadius + maxArcWidth / 2);
        double y = mCenterPoint.y + Math.sin(radian) * (mRadius + maxArcWidth / 2);
        pointPaint.setColor(Color.parseColor("#ebebeb"));
        canvas.drawCircle((float) x, (float) y, mArcWidth + 8, pointPaint);
        pointPaint.setColor(Color.WHITE);
        canvas.drawCircle((float) x, (float) y, mArcWidth + 6, pointPaint);
        canvas.restore();
    }
    ...
    private void updateArcPaint() {
        ...
        SweepGradient fsd = new SweepGradient(mCenterPoint.x, mCenterPoint.y, mBgGradientColors, null);
        mBgArcPaint.setShader(fsd);
    }
