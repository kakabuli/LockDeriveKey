...
public class BleCommandFactory {
     ...
     * @return
     */
    public static byte[] setTimezone(byte zone, byte zoneValue, long time, byte[] pwd1, byte[] pwd3) {
        byte[] data = new byte[6];
        data[0] = zone;
        data[1] = zoneValue;
        byte[] times = littleMode(BleByteUtil.longToUnsigned32Bytes(time));
        System.arraycopy(times, 0, data, 2, times.length);
        return commandPackage(true, (byte) 0x28, commandTSN(), data, pwd1, pwd3);
    }
