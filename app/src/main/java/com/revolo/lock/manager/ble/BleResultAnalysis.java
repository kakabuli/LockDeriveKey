...
public class BleResultAnalysis {
    ...
    public static void processReceivedData(String mac, byte[] receivedData,
                                           byte[] pwd1,
                                           byte[] pwd2Or3,
        ...
        process(mac, receivedData, pwd1, pwd2Or3, bleScanResult);
    }

    public interface OnReceivedProcess {
        void processResult(String mac, BleResultBean bleResultBean);
    }
     ...
     */
    private static void process(String mac, byte[] receivedData, byte[] pwd1, byte[] pwd2Or3, BLEScanResult bleScanResult) {

        if (receivedData.length != 20) {
            Timber.e("getCmd 接收的数据长度不对，不进行解析 length : %1d", receivedData.length);
            return;
        }
        Timber.e("接收的数据 receivedData : %1s", Arrays.toString(receivedData));
        ...
        sOnReceivedProcess.processResult(mac, bleResultBean);
    }
